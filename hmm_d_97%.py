...
def calculate_bic(model, X):
    ...
    n_samples = X.shape[0]
    n_params = (model.n_components - 1) + model.n_components * (model.n_components - 1 + n_features * 2)
...
def time_series_cv(X, n_splits=5):
    ...
    for fold, (train_index, test_index) in enumerate(tscv.split(X), 1):
        ...
        model = hmm.GaussianHMM(n_components=7, covariance_type="full", n_iter=100, random_state=42, verbose=False)
        model.fit(X_train)
